...
function matches(tables: Table, term: string, pipe: PipeTransform) {
    return (tables.name && tables.name.toLowerCase().includes(term))
        || (tables.description && tables.description.toLowerCase().includes(term));
}
...
export class ModuleDemoService {
    ...
    public entity;
    private urlapi = environment.server.url+'/api/base/';
    private httpOptions = {
        headers: new HttpHeaders({ 'Content-Type': 'application/json' })
      };

    constructor(private pipe: DecimalPipe, private httpClient: HttpClient, private serviceBase: BaseService) {
    }
    ...
    private _search(): Observable<SearchResult> {
      ...
      public findAllItems() {
        return this.httpClient.get(this.urlapi+this.entity).pipe(
          map(data => {
      ...
      public _createItem(item: object) {
        return this.serviceBase._createItemEntity(this.entity, item)
      }
    
      public deleteItem(id: number): Observable<any> {
        return this.serviceBase.deleteItemEntity(this.entity, id);
      }
    
      public getItem(id: number): Observable<any> {
        return this.serviceBase.getItemEntity(this.entity, id);
      }
    
      public updateItem(id, item): Observable<any> {
        return this.serviceBase.updateItemEntity(this.entity, id, item);
      }    

      guid() {
        return this.serviceBase.guid();
    }
