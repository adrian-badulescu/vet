...
import { NotacontabilaI, NotacontabiladI } from './accountingnote.model';
import { AccountingnoteService } from './accountingnote.service';
import { BaseService } from './../../_services/base.service';

@Component({
  templateUrl: 'accountingnote.component.html',
  providers: [EwoService, BaseService, AccountingnoteService]
})
export class AccountingnoteComponent extends Base implements OnInit {

  breadCrumbItems: Array<{}>;

  list: NotacontabilaI[];
  columns: any[];
  item: NotacontabilaI = new NotacontabilaI();
  selectedItems: NotacontabilaI[];
  value: Date;

  article: NotacontabiladI;
  article_org: NotacontabiladI;
  invoicexarticles: Array<NotacontabiladI> = [];
  invoicexarticles_deleted: Array<NotacontabiladI> = [];
  ...
  constructor(private serviceBase1: EwoService, private service: BaseService, public formBuilder: FormBuilder) {
    super(serviceBase1);
    this.service.entity = this.entityapi = 'notacontabila';
  }

  ngOnInit() {
    // this.service.entity = this.entityapi = 'notacontabila';
    this.breadCrumbItems = [{ label: 'Dashboard', path: '/' }, { label: 'Contabilitate', path: '/' }, { label: 'Nota contabila', path: '/', active: true }];
  ...
  edit() {
    ...
    this.submitted = false;
    this.item = JSON.parse(JSON.stringify(this.selectedItems)) as NotacontabilaI;
    this.validationform.patchValue(this.item);

    let self = this;
    this.service.findAllItemsEntity('notacontabilad').subscribe(data => {

      data = data.filter(function (entry) {
        return entry['notacontabilaid'] === self.item['id'];
      });
  ...
  onSubmit(values: object) {
    (values['id']) ? this.updateItem(values['id']) : this.createItem(values);
    this.submitted = true;
    ...
    for (var key in this.invoicexarticles) {
      if (this.invoicexarticles[key]['state'] == 'add') {
        ...
        this.service._createItemEntity('notacontabilad', this.invoicexarticles[key]).subscribe(data => {
        });
      }
      if (this.invoicexarticles[key]['state'] == 'edit') {
        console.log(this.invoicexarticles);
        delete this.invoicexarticles[key]['state'];
        this.service.updateItemEntity('notacontabilad', this.invoicexarticles[key]['id'], this.invoicexarticles[key]).subscribe(data => {
        });
      }
    }
    for (var key in this.invoicexarticles_deleted) {
      this.service.deleteItemEntity('notacontabilad', this.invoicexarticles[key]['id']).subscribe(data => {
      });
    }
  }
  ...
  onRowEditInit(item: NotacontabiladI) {
  ...
  onRowEditCancel(item: NotacontabiladI, index: number) {
