...
import {Borderou} from './gestionar.model';
...
export class GestionarComponent extends Base implements OnInit {
    ...
    listBonuriTrimise;listBonuriPredate;
    ...
    trimitebon(sendid){
        ...
        this.listBonuriPredate = this.listBonuriTrimise.filter(function (entry) {
            return entry['sendid'] && entry['sendid'] === sendid;
        });
    ...
    createItem(data) { 
        ...
        if (this.validationform.valid) {
          ...
          this.service._createItemEntity("borderou", this.item).subscribe((data: any) => {
            ...
            this.listBonuriTrimise.unshift(this.item);
            this.updateRowGroupMetaData();
            this.validationform.reset();
