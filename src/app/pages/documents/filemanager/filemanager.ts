...
export class FileManagerComponent implements OnInit {
    ...
    constructor(public ngxSmartModalService: NgxSmartModalService,
        ...
        this.items = [
            ...
            {
                label: 'Archive folder', icon: 'fa fa-folder-open', command: (ev) => {
                    this.clickArchive(this.nodecurrent);
                },
                // disabled: this.nodecurrent ? (this.nodecurrent && this.nodecurrent.isFile === true) : true,
            },
            {
                label: 'Download file', icon: 'fa fa-folder-open', command: (ev) => {
                    this.downloadFile(ev, this.nodecurrent);
                },
                // disabled: this.nodecurrent ? (this.nodecurrent && this.nodecurrent.isFile === false) : true,
            },
        ];
    ...
    clickArchive(node) {
        console.log(node);
        if(!node.isFile){
            alert('Nu poti arhiva fisiere!');
            return;
        }
        this.service.zipFolder(node.id).subscribe((data) => {
            this.getFolders();
            this.expandAll();
            this.buildImages();
        });
    ...
    downloadFile(ev, item){
        console.log(ev);
        if(!item.isFile){
            alert('Nu poti downloada foldere!');
            return;
        }
    ...
    setnodecurrent(node) {
        ...
        if (!node.expanded) {
            ...
            this.service.findAllFolders().subscribe((data: FolderFile[]) => {
                ...
                node.children = data.filter((folder) => folder.parinteId === node.id).map((content) => {
                    if (!content.isFile) {
                        return {
                            label: content.denumire,
                            data: content.description,
        ...
        if (this.nodecurrent) {
            this.service.deleteFolder(this.nodecurrent['id']).subscribe(res => {
                if (this.nodecurrent.isFile) {
                    this.service.deleteFile(this.nodecurrent.nameFile).subscribe((data) => {
                        console.log(data);
                    });
                }
                this.getFolders();
    ...
    savefolder(form) {
        ...
        if (this.editormode == 1) {
            ...
            console.log(this.modalContentLocal);
    ...
    addMultipleFiles() {
        ...
        this.modalContentMultipleFiles =
            {
                parinteId: "",
                description: "",
                isFile: [],
        ...
        this.ngxSmartModalService.getModal('modalMultipleFiles').open();
    }

    saveMultipleFiles(form) {
        console.log(this.modalContentMultipleFiles);
        console.log(this.uploadedFiles);
        // denumire: test, nameFile: guid
        this.uploadedFiles.forEach((file: File, index) => {
            let node = {
                denumire: this.uploadedFiles[index].name,
                parinteId: this.nodecurrent.id,
                isFile: 1,
                nameFile: this.modalContentMultipleFiles.nameFiles[index].file,
                description: this.modalContentMultipleFiles.description
            };
            this.service._createFolder(node).subscribe((data) => {
                this.getFolders();
                this.expandAll();
                this.buildImages();
                // this.setnodecurrent(this.nodecurrent);
            })

        });
        this.ngxSmartModalService.getModal('modalMultipleFiles').close();
        this.uploadedFiles = [];
    }
    ...
    onMultiUpload(event) {
        ...
        this.modalContentMultipleFiles.nameFiles = event.originalEvent.body;
    }
    ...
    nodeExpand(event) {
        if (event.node) {
            this.service.findAllFolders().subscribe((data: FolderFile[]) => {
                event.node.children = (data.filter((folder) => folder.parinteId === event.node.id).map((content) => {
                    if (!content.isFile) {
                        return {
                            label: content.denumire,
                            data: content.description,
