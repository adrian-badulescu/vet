...
export class FilemanagerService{
    ...
    public deleteFolder(id: string): Observable<FolderFile> {
        return this.httpClient.delete<FolderFile[]>(`${this.folderApiURL}/${id}`, this.httpOptions).
          pipe(
    ...
    deleteFile(name: string): Observable<any> {
        return this.httpClient.delete<any>(environment.server.url+`/api/base/files/${name}`, this.httpOptions).
        pipe(
    ...
    public updateFolder(id, folder): Observable<FolderFile> {
      ...
      return this.httpClient.put<FolderFile>(`${this.folderApiURL}/${id}`, folder, this.httpOptions).
      pipe(
    ...
    getFile(fileID): Observable<any> {
        return this.httpClient.get<any>(environment.server.url+`/api/base/files/${fileID}`, {...this.httpOptions, responseType: 'blob' as 'json'}).
        pipe(
    ...
    public getFolderById(id: number): Observable<FolderFile> {
      return this.httpClient.get<FolderFile>(`${this.folderApiURL}/${id}`).
        pipe(
          tap((folder: FolderFile) => console.log(`fetched folder id=${id}`)),
