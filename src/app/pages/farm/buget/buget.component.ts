...
import { BugetCls } from './buget.model';
...
@Component({
  ...
  providers: [EwoService, BaseService, BooleanPipe]
})
export class BugetComponent extends Base implements OnInit {
  ...
  list: BugetCls[];
  columns: any[];
  item: BugetCls = new BugetCls();
  selectedItems: BugetCls[];
  value: Date;
  ...
  BudgetCenters: Array<string> = ['A', 'B', 'C'];
  ...
  ngOnInit() {
    this.service.entity = this.entityapi = 'buget';
    this.validationform = this.formBuilder.group({
      ...
      budgetCenters: ['', [Validators.required]],
      generalBudget: ['', [Validators.required]],
    });
  ...
  edit() {
    ...
    this.submitted = false;
    this.item = JSON.parse(JSON.stringify(this.selectedItems)) as BugetCls;
