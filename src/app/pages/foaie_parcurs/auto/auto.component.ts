...
import { AutoCls, AutodCls } from './auto.model';
...
export class AutoComponent extends Base implements OnInit {

  breadCrumbItems: Array<{}>;

  list: AutoCls[];
  columns: any[];
  item: AutoCls = new AutoCls();
  selectedItems: AutoCls[];
  value: Date;
  FuelType: Array<string> = ['Benzina', 'Motorina', 'Kerosen', 'H2O', '...',];
  article: AutodCls;
  article_org: AutodCls;
  invoicexarticles: Array<AutodCls> = [];
  invoicexarticles_deleted: Array<AutodCls> = [];
  ...
  ngOnInit() {
    this.service.entity = this.entityapi = 'auto';
  ...
  edit() {
    ...
    this.submitted = false;
    this.item = JSON.parse(JSON.stringify(this.selectedItems)) as AutoCls;
    ...
    this.service.findAllItemsEntity('invoicexarticles').subscribe(data => {
      ...
      data = data.filter(function (entry) {
        return entry['autoid'] === self.item['id'];
      });
  ...
  onRowEditCancel(item: AutodCls, index: number) {
