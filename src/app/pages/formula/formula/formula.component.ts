...
import { FormulaCls, FormuladCls } from './formula.model';
import { FormulaService } from './formula.service';
import { BaseService } from '../../_services/base.service';

@Component({
  templateUrl: 'formula.component.html',
  providers: [EwoService, BaseService, FormulaService]
})
export class FormulaComponent extends Base implements OnInit {

  breadCrumbItems: Array<{}>;

  list: FormulaCls[];
  columns: any[];
  item: FormulaCls = new FormulaCls();
  selectedItems: FormulaCls[];
  value: Date;

  article: FormuladCls;
  article_org: FormuladCls;
  invoicexarticles: Array<FormuladCls> = [];
  invoicexarticles_deleted: Array<FormuladCls> = [];
  ...
  ngOnInit() {
    this.service.entity = this.entityapi = 'formula';
    ...
    this.service.findAllItemsEntity('formulad').subscribe(data => {
  ...
  edit() {
    ...
    this.submitted = false;
    this.item = JSON.parse(JSON.stringify(this.selectedItems)) as FormulaCls;
    ...
    this.service.findAllItemsEntity('formulad').subscribe(data => {
      ...
      data = data.filter(function (entry) {
        return entry['formulaid'] = self.item['id'];
      });
  ...
  onSubmit(values: object) {
    ...
    for (var key in this.invoicexarticles) {
      if (this.invoicexarticles[key]['state'] == 'add') {
        ...
        this.service._createItemEntity('formulad', this.invoicexarticles[key]).subscribe(data => {
        });
      }
      if (this.invoicexarticles[key]['state'] == 'edit') {
        console.log(this.invoicexarticles);
        delete this.invoicexarticles[key]['state'];
        this.service.updateItemEntity('formulad', this.invoicexarticles[key]['id'], this.invoicexarticles[key]).subscribe(data => {
        });
      }
    }
    for (var key in this.invoicexarticles_deleted) {
      this.service.deleteItemEntity('formulad', this.invoicexarticles[key]['id']).subscribe(data => {
      });
    }
  }
  ...
  onRowEditInit(item: FormuladCls) {
  ...
  onRowEditCancel(item: FormuladCls, index: number) {
