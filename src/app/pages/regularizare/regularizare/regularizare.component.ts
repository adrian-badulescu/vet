...
import { RegularizareCls, RegularizaredCls } from './regularizare.model';
import { RegularizareService } from './regularizare.service';
import { BaseService } from './../../_services/base.service';

@Component({
  templateUrl: 'regularizare.component.html',
  providers: [EwoService, BaseService, RegularizareService]
})
export class RegularizareComponent extends Base implements OnInit {

  breadCrumbItems: Array<{}>;

  list: RegularizareCls[];
  columns: any[];
  item: RegularizareCls = new RegularizareCls();
  selectedItems: RegularizareCls[];
  value: Date;
  Administration: Array<string> = ['A', 'B', 'C', '...'];
  article: RegularizaredCls;
  article_org: RegularizaredCls;
  invoicexarticles: Array<RegularizaredCls> = [];
  invoicexarticles_deleted: Array<RegularizaredCls> = [];
  ...
  ngOnInit() {
    this.service.entity = this.entityapi = 'regularizare';
    ...
    this.validationform = this.formBuilder.group({
      ...
      adjustmentMotivation: ['', [Validators.required]]
    });
  ...
  edit() {
    ...
    this.submitted = false;
    this.item = JSON.parse(JSON.stringify(this.selectedItems)) as RegularizareCls;
    this.validationform.patchValue(this.item);

    let self = this;
    this.service.findAllItemsEntity('regularizared').subscribe(data => {
  ...
  onSubmit(values: object) {
    ...
    for (var key in this.invoicexarticles) {
      if (this.invoicexarticles[key]['state'] == 'add') {
        ...
        this.service._createItemEntity('regularizared', this.invoicexarticles[key]).subscribe(data => {
        });
      }
      if (this.invoicexarticles[key]['state'] == 'edit') {
        console.log(this.invoicexarticles);
        delete this.invoicexarticles[key]['state'];
        this.service.updateItemEntity('regularizared', this.invoicexarticles[key]['id'], this.invoicexarticles[key]).subscribe(data => {
        });
      }
    }
    for (var key in this.invoicexarticles_deleted) {
      this.service.deleteItemEntity('regularizared', this.invoicexarticles[key]['id']).subscribe(data => {
      });
    }
  }
  ...
  onRowEditInit(item: RegularizaredCls) {
  ...
  onRowEditCancel(item: RegularizaredCls, index: number) {
