...
import { Table, SearchResult } from './receptie.model';
...
function matches(tables: Table, term: string, pipe: PipeTransform) {
  ...
  return pipe.transform(tables.id).includes(term)    
    || pipe.transform(tables.number).includes(term)
    || pipe.transform(tables.date).includes(term)
...
export class ReceptieService {
  ...
  private inventoryDb = environment.server.url+"/api/receptie";
